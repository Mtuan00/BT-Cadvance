...
void xapxeptubedenlon(struct typeArr value)
{
	int tg;
...
void xapxeptulondenbe(struct typeArr value)
{
	int tg;
    for(int i = 0; i < value.size; i++){
        for(int j = i + 1; j < value.size; j++){
...
{
    static struct typeArr arr;//luwu dia chi co dinh ton tai dden khi ket thuc truong chinh
...
void xuatthongtin(struct typeArr value)
{
	for (int i = 0; i < value.size; i++)
    {
        printf("xuat[%d]: %d\n",i,value.arr[i]);
